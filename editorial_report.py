...
CATEGORY_FIT = {
    "art_history": 1.35,
    "art_counterculture": 1.40,
    "art_memes": 1.25,
    "soviet_art": 1.30,
    "visual_culture": 1.20,
    "culture": 1.25,
    "culture_philosophy": 1.30,
    "culture_science_society": 1.25,
    "culture_media": 1.10,
    "technology": 1.30,
    "technology_culture": 1.40,
    "technology_science": 1.20,
    "technology_internet_culture": 1.35,
    "technology_memes": 1.20,
    "philosophy_art": 1.25,
    "history_society": 1.15,
    "fashion_history": 1.20,
    "fashion_culture": 1.15,
    "cinema": 1.25,
    "architecture_history": 1.15,
    "marketing": 1.10,
    "viral": 0.78,
    "news": 0.65,
    "breaking_news": 0.48,
...
def topic_confidence(topic: dict[str, Any]) -> float:
    ...
    confidence = 0.0
    confidence += min(sources, 4) * 0.8
    confidence += min(posts, 5) * 0.25
    ...
    if len(words) == 1 and kind != "entity":
        confidence -= 3.0
...
def topic_title(topic: dict[str, Any]) -> str:
    term = str(topic.get("term") or "").strip()
    words = [word.lower().replace("ё", "е") for word in WORD_RE.findall(term)]
    generic_ratio = (sum(word in GENERIC_TOPIC_WORDS for word in words) / len(words)) if words else 1.0
    if generic_ratio <= 0.34:
        return term
    examples = topic.get("examples") or []
    if examples:
        text = str(examples[0].get("text") or "").strip()
        sentence = re.split(r"[.!?\n]", text, maxsplit=1)[0].strip(" —–-")
        if 12 <= len(sentence) <= 105:
            return sentence
    return term
...
def editorial_reason(post: dict[str, Any]) -> str:
    ...
    if re.search(r"искусств|худож|галере|музе|авангард|архитектур", text, re.I):
        return "искусство / культурный контекст"
    if re.search(r"нейросет|искусственн\w+ интеллект|\bAI\b|модель|опенсорс|open.?source|робот", text, re.I):
        return "AI / технология"
    if re.search(r"кино|фильм|сериал|режисс|акт[её]р", text, re.I):
        return "кино / поп-культура"
    if re.search(r"эксперимент|исследован|уч[её]н", text, re.I):
...
def dedupe_posts(posts: list[dict[str, Any]], limit: int) -> list[dict[str, Any]]:
    ...
    for post in posts:
        ...
        if per_channel.get(channel, 0) >= 2:
            continue
        prefix = re.sub(r"\W+", "", str(post.get("text") or "").lower())[:90]
...
def real_trends(topics: list[dict[str, Any]], limit: int = 15) -> list[dict[str, Any]]:
    ...
    for topic in topics:
        ...
        if int(topic.get("source_count") or 0) < 2:
            continue
        if confidence < 3.2:
            continue
        row = dict(topic)
        ...
        rows.append(row)
    rows.sort(
        key=lambda row: (row["confidence"], float(row.get("score") or 0), int(row.get("views") or 0)),
        reverse=True,
    )
    return rows[:limit]
...
def build_report(snapshot: dict[str, Any]) -> str:
    ...
    if topics:
        for topic in topics:
            lines.append(
                f"| {esc(topic['display_title'])} | {topic.get('source_count', 0)} | "
                f"{topic.get('post_count', 0)} | {topic.get('views', 0)} | "
                f"{topic['confidence']:.2f} | {examples_md(topic)} |"
            )
    else:
    ...
    lines += [
        ...
        "## Стоит написать пост",
        "",
        "| Канал | Fit | Просмотры | Возраст, ч | Почему интересно | Публикация |",
        "|---|---:|---:|---:|---|---|",
    ]
    for post in picks:
        ...
        text = esc(post.get("text"))[:260]
        reason = editorial_reason(post)
        lines.append(
            f"| {channel} | {post['content_fit']:.2f} | {post.get('views', 0)} | "
            f"{float(post.get('age_hours') or 0):.1f} | {reason} | {text} |"
        )
    ...
    lines += [
        ...
        "## Просто горячее",
        "",
        "| Канал | Сигнал | Просмотры | Возраст, ч | Публикация |",
        "|---|---:|---:|---:|---|",
    ]
    for post in hot:
        title = esc(post.get("channel_title") or post.get("channel"))
        link = post.get("link")
        channel = f"[{title}]({link})" if link else title
        text = esc(post.get("text"))[:220]
        lines.append(
            f"| {channel} | {float(post.get('signal_score') or 0):.3f} | {post.get('views', 0)} | "
            f"{float(post.get('age_hours') or 0):.1f} | {text} |"
        )
